...
class Blip2T5Instruct(Blip2Base):
    ...
    def generate(
        ...
        if "qa_prompt" in samples.keys():
            qa_prompt = samples["qa_prompt"]
        else:
            qa_prompt = self.qa_prompt
        
        image, qid = samples["video"], samples['question_id']
        text_input_qa, answer, question  = samples['qa_input'], samples['qa_output'], samples['question_only']

        #QA Template
        for i in range(len(text_input_qa)):
            text_input_qa[i] = text_input_qa [i] + " "+ qa_prompt

        b, t, c, w, h = image.shape
        image = image.reshape(-1, c, w, h)
            
        #to check if promtp is str
        if isinstance(prompt, str):
            prompt = [prompt] * b
        else:
            assert len(prompt) == b, "The number of prompts must be equal to the batch size."
        
        #Question-Dependent Instructional Template
        for i in range(len(question)):
            prompt[i] = prompt [i] + " " + question[i]
        ...
        if self.qformer_text_input:
            ...
            text_Qformer_id = torch.repeat_interleave(text_Qformer.input_ids,t,0)
            text_Qformer_atts = torch.repeat_interleave(text_Qformer.attention_mask,t,0)
            Qformer_atts = torch.cat([query_atts,text_Qformer_atts],dim=1)
        else:
            raise ValueError("An input prompt for the Qformer is needed")


        with self.maybe_autocast():
            frame_embeds = self.ln_vision(self.visual_encoder(image))
        ...
        input_tokens_id = torch.repeat_interleave(input_tokens.input_ids,t,0)
        input_tokens_atts = torch.repeat_interleave(input_tokens.attention_mask,t,0)

        encoder_atts = torch.cat([frame_atts_t5, input_tokens_atts], dim=1)
        
        with self.maybe_autocast(dtype=torch.bfloat16):
            inputs_embeds = self.t5_model.encoder.embed_tokens(input_tokens_id)
            inputs_embeds = torch.cat([frame_inputs_t5, inputs_embeds], dim=1)
            ...
            output_caps = self.t5_tokenizer.batch_decode(
                outputs, skip_special_tokens=True
            )

        #List of captions
        caption_tokens = self.t5_tokenizer(
            output_caps, padding="longest", add_special_tokens=False, 
            truncation=True,max_length=self.max_txt_len, return_tensors="pt").to(image.device)
        
        caption_tokens_mask=torch.reshape(caption_tokens.attention_mask,(b,-1)) 

        #Captions Prefix
        cap_prefix = self.t5_tokenizer(
            self.capt_prefix, padding="longest", add_special_tokens=False,
            truncation=True, max_length=self.max_txt_len, return_tensors="pt").to(image.device)
        
        cap_prefix_id = torch.repeat_interleave(cap_prefix.input_ids, b, 0)
        cap_prefix_mask = torch.repeat_interleave(cap_prefix.attention_mask, b, 0)

        #QA prompt
        input_prompt_qa = self.t5_tokenizer(
        ...
        with self.maybe_autocast(dtype=torch.bfloat16):
            ...
            cap_prefix_embeds = self.t5_model.encoder.embed_tokens(cap_prefix_id)
            caption_embeds = self.t5_model.encoder.embed_tokens(caption_tokens.input_ids)
            caption_list_embeds = torch.reshape(caption_embeds,(b,-1,cap_prefix_embeds.shape[-1]))
            inputs_embeds_qa = self.t5_model.encoder.embed_tokens(input_prompt_qa.input_ids)

            inputs_embeds = torch.cat([cap_prefix_embeds, caption_list_embeds, inputs_embeds_qa], dim=1) 
            encoder_atts = torch.cat([cap_prefix_mask, caption_tokens_mask, input_prompt_qa.attention_mask], dim=1)
            ...
            pred_logits_qa = outputs.scores[0]
            pred_logits_qa = pred_logits_qa[:, self.answer_id] 
    ...
    def from_config(cls, cfg):
        ...
        prompt = cfg.get("cap_prompt", "")
        qa_prompt = cfg.get("qa_prompt","")
