...
class SeViLA(Blip2Base):
    ...
    def __init__( self, img_size=224, drop_path_rate=0,
        ...
        num_query_token=32, t5_model="google/flan-t5-xxl", prompt="",
        ...
        self.sum_prefix = ['Captions: ']
        self.desc_prefix = ['Combine the information into a single sentence that compiles all the information: ']
        self.num_summaries = 5
    ...
    def generate(self,
        ...
        text_input_qa, answer, cap_prompt  = samples['qa_input'], samples['qa_output'], samples['question_input']
        ...
        if 'loc' not in self.task or 'uni_eval' in self.task:
            ...
            with torch.cuda.amp.autocast(dtype=torch.bfloat16):
                ...
                outputs_qa = self.t5_model.generate(
                    inputs_embeds=inputs_t5_qa, attention_mask=atts_t5_qa,
        ...
        # inference with localizer             
            ...
            image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(image.device) # bt n c
            image_embeds_, image_atts_ = image_embeds.detach().clone(), image_atts.detach().clone()
            image_embeds_ = self.ln_vision_loc(image_embeds_) #(1,32,768)
            
            text_input_loc = samples['loc_input'] # Q + Prompt: Is this a good frame can answer the question?

            query_tokens_loc = self.query_tokens_loc.expand(image_embeds_.shape[0], -1, -1) #(150,32,768)           

            query_output_loc = self.Qformer_loc.bert(
                query_embeds=query_tokens_loc, encoder_hidden_states=image_embeds_,
                encoder_attention_mask=image_atts_, return_dict=True)
            inputs_t5_loc = self.t5_proj_loc(query_output_loc.last_hidden_state)
            atts_t5_loc = torch.ones(inputs_t5_loc.size()[:-1], dtype=torch.long).to(image.device) #(150,32)

            with torch.cuda.amp.autocast(dtype=torch.bfloat16):
                
                question_tokens_cap = self.t5_tokenizer(
                    cap_prompt, padding="longest", truncation=True,
                    max_length=self.max_txt_len, return_tensors="pt").to(image.device)
                question_prefix_id = torch.repeat_interleave(question_tokens_cap.input_ids, t, 0) #(150,57)
                question_prefix_mask = torch.repeat_interleave(question_tokens_cap.attention_mask, t, 0)

                question_embeds_qa = self.t5_model.encoder.embed_tokens(question_prefix_id)
                inputs_embeds = torch.cat([inputs_t5_loc, question_embeds_qa], dim=1) #(2,1505,2048) Embeddings concatenation
                encoder_atts = torch.cat([atts_t5_loc, question_prefix_mask], dim=1) #(2,1505) Mask concatenation

                outputs_loc = self.t5_model.generate(
                    inputs_embeds=inputs_embeds, attention_mask=encoder_atts,
                    do_sample=True, top_p=0.7, temperature=temperature, num_beams=1,
                    max_new_tokens=30, min_length=1, repetition_penalty=2.0,
                    length_penalty=length_penalty, num_return_sequences=num_captions,
                    return_dict_in_generate=True, output_hidden_states=True, output_scores=True)

                caption = self.t5_tokenizer.batch_decode(outputs_loc.sequences,skip_special_tokens=True)

                #List of captions
                caption_tokens = self.t5_tokenizer(
                    caption, padding="longest", add_special_tokens=False, 
                    truncation=True,max_length=self.max_txt_len, return_tensors="pt",
                    ).to(image.device) #(150,21)

                caption_tokens_mask=torch.reshape(caption_tokens.attention_mask,(b*self.num_summaries,-1))
                ...
                sum_prefix_id = torch.repeat_interleave(sum_prefix.input_ids, self.num_summaries*b, 0) #(2,4)
                sum_prefix_mask = torch.repeat_interleave(sum_prefix.attention_mask, self.num_summaries*b, 0)

                #Embeddings and Final Concatenations
                sum_prefix_embeds = self.t5_model.encoder.embed_tokens(sum_prefix_id) #(50,3,2048) 
                caption_embeds = self.t5_model.encoder.embed_tokens(caption_tokens.input_ids) #(150,30,2048)
                caption_list_embeds = torch.reshape(caption_embeds,(b*self.num_summaries,-1,caption_embeds.shape[-1])) #(50,90,2048)

                inputs_embeds = torch.cat([sum_prefix_embeds, caption_list_embeds], dim=1) #Embeddings concatenation (50,93,2048)
                encoder_atts = torch.cat([sum_prefix_mask, caption_tokens_mask], dim=1) #Mask concatenation
                ...
                outputs_qa = self.t5_model.generate(
                    ...
                    temperature=1, num_beams=1,
                    max_new_tokens=30, min_length=1,
                    repetition_penalty=2.0, length_penalty=1.0,
                    num_return_sequences=1, return_dict_in_generate=True,
                    output_hidden_states=False, output_scores=True)            
            
                #Response
                summary = self.t5_tokenizer.batch_decode(outputs_qa.sequences,skip_special_tokens=True)

                #List of summaries
                sum_tokens = self.t5_tokenizer(
                    summary, padding="longest", add_special_tokens=False, 
                    truncation=True,max_length=self.max_txt_len, return_tensors="pt",
                    ).to(image.device) #(150,21)

                sum_tokens_mask=torch.reshape(sum_tokens.attention_mask,(b,-1)) 

                #Captions Prefix
                cap_prefix = self.t5_tokenizer(
                    self.sum_prefix, padding="longest", add_special_tokens=False,
                    truncation=True, max_length=self.max_txt_len, return_tensors="pt",
                    ).to(image.device)

                cap_prefix_id = torch.repeat_interleave(cap_prefix.input_ids, b, 0) #(2,4)
                cap_prefix_mask = torch.repeat_interleave(cap_prefix.attention_mask, b, 0)
                ...
                #Embeddings and Final Concatenations
                inputs_embeds_qa = self.t5_model.encoder.embed_tokens(input_tokens_qa.input_ids) #(2,77,2048)
                cap_prefix_embeds = self.t5_model.encoder.embed_tokens(cap_prefix_id) #(2,4,2048)
                sum_embeds = self.t5_model.encoder.embed_tokens(sum_tokens.input_ids) #(50,30,2048)
                sum_resh_embeds = torch.reshape(sum_embeds,(b,-1,cap_prefix_embeds.shape[-1])) #(2,750,2048)

                inputs_embeds = torch.cat([cap_prefix_embeds, sum_resh_embeds, inputs_embeds_qa], dim=1) #Embeddings concatenation (2,831,2048)
                encoder_atts = torch.cat([cap_prefix_mask, sum_tokens_mask, input_tokens_qa.attention_mask], dim=1) #Mask concatenation (2,831)
                ...
                #Response
                #response = self.t5_tokenizer.batch_decode(outputs_qa.sequences,skip_special_tokens=True)
