...
class MCVideoQADataset(MultimodalClassificationDataset, __DisplMixin):
    ...
    def __getitem__(self, index):
        ...
        while result is None:
            ...
            if 'start' in ann:
                clip=None
                #tart, end = float(ann['start']), float(ann['end'])
                #clip = [start, end]
            else:
                clip = None       
            ...
            if 'VLEP' in qid:
                ...
                loc_prompt = 'Question: ' + q +  ' ' + pseudo_options + ' Which frames are related in any way with the information in the prompt? Choose an option for each frame.'
                ...
                for j in range(ann['num_option']):
                    ...
                    hints += ' '
                hints += ')'
                #qa_prompt = prompt + ' Considering the information presented in the captions, select the correct answer in one letter (A,B,C,D,E) from the options.'
            ...
            try:
                if 'VLEP' in qid:
                    ...
                    if ':' in video_id:
                        ...
                        video_path = 'vlep_ytb_clips_tars/videos/vlep_ytb_clips/'
                    else:
                        video_id = video_id[:-3]
                        video_path = 'videos/tvqa/videos_3fps_with_audio/'
                ...
                print("Asked Frames",len(frms))
                print("Actual Frames",self.vis_processor.n_frms)
                index = random.randint(0, len(self.annotation) - 1)
                
        return {
            "video": frms,
            "qa_input": prompt,
            "question_only": 'Question: ' + q ,
            "question_input": "A photo of: ",
